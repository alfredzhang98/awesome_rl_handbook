...
| | |
|---|---|
| **上一步的极限** | 前一课那套办法在哪儿撑不住了（要具体：哪个前提不成立、代价是多少） |
...
- 引入一个新记号时同时给：**读法**（怎么念）、**类型**（是数、是向量、是函数还是算子）、**取值范围**。
- 公式里出现的每一项，正文里都要有一句话解释它管什么。整式给完之后，用下花括号标注或紧跟一段逐项拆解。
- **推导不跳步**。允许省略的只有纯代数变形，且要注明「两边同乘 …」。凡是"显然"「容易看出」这类词，都要换成实际的那一步。
...
场景式「一间仓库，一张表」；评论式「三件套的账：轮数少 ≠ 算得少」；口头禅式「先把话说白」。
...
- **不要口头禅**：「先把 X 说死」「先把话说白」「说穿了」这类腔调一律不用，直接给内容。
