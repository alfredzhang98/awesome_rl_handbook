...
标题（h1、小节名、路线图 Stage 名、图里的格子、侧栏短名）**一律用能出现在教材目录里的术语，单行，不加副标题**。

| 该写 | 不该写 |
|---|---|
| 值函数 V 与最优值函数 V⋆ | ~~V 到底是什么，以及 V⋆ 特别在哪~~ |
| 马尔可夫决策过程与 Bellman 算子 | ~~一间仓库，一张表~~ |
| 动态规划：策略评估、策略改进与值迭代 | ~~把值表算出来 / 动态规划三件套~~ |
| 无模型价值方法：从 Q-learning 到 DQN | ~~不给模型，只能靠试~~ |
| 值函数逼近 | ~~函数逼近与致命三角 / 稳定性去哪了~~ |

正文里可以生动，标题不行。设问式、动作式、场景式的名字一个都不要。
