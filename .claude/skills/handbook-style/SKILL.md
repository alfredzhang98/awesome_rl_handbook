---
name: handbook-style
description: 写「数学优先、单文件 HTML、可交互」的中文技术讲义时使用——每课一个能手算的小例子，先给能手算的数字再给公式，图和数字全部由脚本跑出来。触发词：讲义、handbook、学习手册、教程页面、单文件 HTML 文档、把某个理论从头讲清楚、RL/控制/数学笔记。也用于给已有的这类讲义补写新一课、加交互组件、加图、切分成书。
---
...
- 卡片里：`.stage-head`（`STAGE N` 徽章 + 标题 + 跳转到对应课的 `.chip`）→ `.why`（**为什么必须学这一段**，一句狠话）→ `.block h4` 分组（核心算法 / 核心内容）→ `ul.algos` 逐条 → `.demo`（本课例子的落点）→ `.pitfall`（这一段最容易踩的坑）。
